...
// [12, 14]
console.log("--------------------");
console.log("ARRAYS.JS CHALLENGES: ")
...
carModelsSorted = modelsOnly(inventory).sort();
...
function yearsOnly(arr) {
  for (let i=0; i<arr.length; i++) {
    if (carYears.includes(inventory[i].car_year) == false) {
...
carYears = yearsOnly(inventory).sort();
...
function select(arr) {
  let final = [];
  for (let i=0; i<arr.length; i++) {
    current = arr[i].car_make;
    currentObj = arr[i];
    if (current == "BMW" || current == "Audi") {
      final.push(currentObj);
    }
  }
  return final;
}

BMWAndAudi = select(inventory);
